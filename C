...
function fmtTime(dateStr: string): string {
  return formatInTimeZone(
    toZonedTime(new Date(dateStr), KKN_CONFIG.TIMEZONE),
    'HH:mm',
    { timeZone: KKN_CONFIG.TIMEZONE },
  );
}
...
/** 40-day monitoring rows indexed by day number. */
...
function buildDaySummaries(attendances: AttendanceWithProfile[]): DaySummary[] {
  ...
  for (const a of attendances) {
    const dayIndex = days.findIndex(
      (d) => d.date === fmtAttendanceDate(a.attendance_date),
    );
    if (dayIndex !== -1) {
...
function buildDataAbsensiRows(attendances: AttendanceWithProfile[], start: string, end: string): string[][] {
  const rows: string[][] = [
    ...headerRows(start, end),
    [],
    ['No', 'Nama', 'NIM', 'Fakultas', 'Jurusan', 'Tanggal', 'Jam', 'Status', 'Catatan'],
...
function buildRekapAnggotaRows(byMember: MemberReport[], start: string, end: string): string[][] {
  const rows: string[][] = [
    ...headerRows(start, end),
    [],
...
function buildMonitoringRows(attendances: AttendanceWithProfile[], start: string, end: string): string[][] {
